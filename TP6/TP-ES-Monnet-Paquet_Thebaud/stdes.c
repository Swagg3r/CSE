...
//fonction qui retourne la chaine de caractere associé a l'entier passé en parametre. equivalent de itoa()
char* conversion (int entier){
	char* retour;
	int copie = entier;
	int i=1;
	int pos=0;
	//Test le nombre de caractere dans le int. Ex : 123 = 3 caracteres.
	while(copie){
		copie = copie/10;
		i++;
	}
	//Teste si l'entier est positif pour ajouter une case pour le '-'
	if (entier < 0){
		i++;
		pos = 1;
	}
	...
	//On va ajouter dans le tableau en partant de la fin.
	pos = i-2;
	//Aout du caractere de fin de chaine.
	retour[i-1] = '\0';
	//Tant que la division : entier/10 > 0 on recupere le dernier chiffre du int passé en param avec un modulo.
	while (entier){
		retour[pos] = '0'+entier%10;
		entier = entier/10;
		pos--;
	}
	return retour;
